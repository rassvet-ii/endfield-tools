...
const $id = <T extends string>(ids: readonly { id: T }[]) =>
  z
    .looseObject({ id: identifier.and(z.enum(ids.map((it) => it.id))) })
    .transform((item) => item.id)
    .array()
    .superRefine((items, ctx) => {
      checkUnique(
        ctx,
        items.map((item, index) => ({ path: [index, 'id'], value: item })),
      );
    })
    .transform((ids) => {
...
export const $ids = z.strictObject({
  stats: z
    .strictObject({
      attribute: $id(static_data.stats.attribute),
      secondary: $id(static_data.stats.secondary),
      skill: $id(static_data.stats.skill),
    })
    .superRefine((stats, ctx) => {
      const entries = Object.entries(stats).flatMap(([category, items]) =>
        items.map((item, index) => ({ path: [category, index, 'id'], value: item })),
      );
      checkUnique(ctx, entries);
    }),
  energy_alluviums: $id(static_data.energy_alluviums),
  weapons: $id(sync_data.weapons),
  operators: $id(sync_data.operators),
  elements: $id(static_data.elements),
  operator_classes: $id(static_data.operator_classes),
  weapon_types: $id(static_data.weapon_types),
});

export const $database = (ids: z.infer<typeof $ids>) =>
  z.strictObject({
...
export const id = $ids.parse(merged_data);
